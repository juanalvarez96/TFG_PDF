...
\begin{figure}
	...
	\caption{The Model~\cite{model}}
	\label{fig:model}
\end{figure}
\par
\Cref{fig:model} shows the steps followed during the~\ac{ml} stage.\\
The first three steps are summarized in~\cref{sec:use-cases}. Training the algorithm is done in~\cref{sec:feature} and~\cref{sec:clasif}. Finally, the last three steps are outlined in~\cref{sec:score}.
\section{Dataset}
\label{sec:use-cases}
